...
os.environ["LANGCHAIN_PROJECT"] = "week9-deployment"
...
# ============= 상태 업데이트 콜백 =============
def update_status(run_id: str, status: str, step: int = None, result: dict = None, error: str = None):
    """파이프라인 상태 업데이트 콜백"""
...
async def health_check():
    """서비스 상태 확인"""
    active_count = len([s for s in pipeline_status.values() if s["status"] == "running"])
